...
export default function TriagePanel() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm" onClick={() => setOpen(false)}>
      <div
        className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-2xl"
        ...
        {!proposals ? (
          <div className="flex flex-col items-center justify-center gap-3 p-8 text-center">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-md bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-700 disabled:opacity-50"
            >
              {running ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
              Triage Inbox
            </button>
            {inbox.length === 0 && (
              <div className="flex items-center gap-1 rounded-md border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs text-amber-800">
                  ...
                  return (
                    <li
                      ...
                      className={`rounded-md border px-3 py-2 ${
                        isAccepted ? 'border-violet-200 bg-violet-50/40' : 'border-gray-200 bg-white'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 text-[11px] italic text-gray-500">{p.reason}</div>
                        </div>
                        <button
                          type="button"
...
function Diff({
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-1.5 py-0.5 text-gray-600">
      <span className="text-[10px] uppercase tracking-wider text-gray-500">{field}:</span>
