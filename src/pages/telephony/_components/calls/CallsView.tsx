...
function OutcomeChip({ outcome, status }: { outcome: CallOutcome; status: CallStatus }) {
  if (outcome) {
    const styles: Record<NonNullable<CallOutcome>, string> = {
      ...
      user_aborted: 'bg-gray-100 text-gray-500',
      ...
      >
        ...
        {placed ? (
          <div className="flex flex-col items-center justify-center px-6 py-12 text-center">
            ...
            <p className="mt-1 text-sm text-gray-400">Agent is {when === 'now' ? 'dialling…' : 'ready for ' + schedDate}</p>
          </div>
        ) : step === 1 ? (
          <div className="space-y-4 px-6 py-5">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select className="rounded-lg border border-gray-200 bg-gray-50 px-2 py-2 text-sm outline-none focus:border-[#8fc4e4]">
                  <option>🇩🇪 +49</option>
                  <option>🇦🇹 +43</option>
                  <option>🇨🇭 +41</option>
                </select>
                <input
                  ...
                  className="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none placeholder:text-gray-400 focus:border-[#8fc4e4]"
                />
              </div>
            </div>
            ...
            {/* Contact name */}
            <div>
              <label className="mb-1.5 block text-xs font-medium text-gray-600">Contact name (optional)</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-200 py-2 pl-8 pr-3 text-sm outline-none placeholder:text-gray-400 focus:border-[#8fc4e4]"
                />
              </div>
            </div>
            ...
            {/* Agent */}
            <div>
              <label className="mb-1.5 block text-xs font-medium text-gray-600">Agent</label>
              <div className="relative">
                <select
                  value={agent}
                  onChange={(e) => setAgent(e.target.value)}
                  className="w-full appearance-none rounded-lg border border-gray-200 bg-white px-3 py-2 pr-8 text-sm outline-none focus:border-[#8fc4e4]"
                >
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-gray-600">
                Objective *
                <span className="ml-1 font-normal text-gray-400">(what should the agent do?)</span>
              </label>
              <textarea
                ...
                rows={3}
                className="w-full resize-none rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none placeholder:text-gray-400 focus:border-[#8fc4e4]"
              />
            </div>
          ...
          <div className="space-y-4 px-6 py-5">
            ...
            <div className="rounded-xl bg-gray-50 p-4 text-sm">
              <p className="font-medium text-gray-900">{contactName || phone}</p>
              <p className="mt-0.5 text-xs text-gray-500">{phone}</p>
              <p className="mt-2 text-xs text-gray-700">{objective}</p>
              <div className="mt-2 flex items-center gap-2">
                <span className="rounded-full bg-[#dde9f4] px-2 py-0.5 text-[10px] text-gray-700">{agent}</span>
                {priority === 'urgent' && (
            ...
            {/* When */}
            <div>
              <label className="mb-2 block text-xs font-medium text-gray-600">When</label>
              <div className="flex gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#8fc4e4]"
                  />
                </div>
                <div>
            ...
            <div className="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-500">
              ...
              Estimated cost: <span className="font-medium text-gray-700">€0.35 – €0.85</span>
              <span className="ml-auto text-[10px] text-gray-400">based on avg {agent} duration</span>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
            ...
            {step === 1 ? (
              <button
                ...
                disabled={!canNext}
                onClick={() => setStep(2)}
                className="rounded-full bg-[#bdd8ec] px-4 py-2 text-sm font-medium text-gray-800 transition-colors hover:bg-[#a5c8e0] disabled:opacity-40"
              >
                Next →
              </button>
            ) : (
    ...
    <div className="flex-1 overflow-y-auto p-4">
      {byDay.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-gray-400">No scheduled calls</p>
        </div>
      ) : (
        <div className="space-y-6">
          {byDay.map(([day, calls]) => (
            <div key={day}>
              <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-400">{day}</p>
                ...
                {calls.map((call) => (
                  ...
                  >
                    ...
                    <div className="w-14 shrink-0 text-center">
                      ...
                      <p className="text-[10px] text-gray-400">Uhr</p>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="rounded-full bg-[#dde9f4] px-2 py-0.5 text-[10px] text-gray-700">
                          {call.agentPersona}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-gray-500 truncate">{call.objective}</p>
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                      <button
                        type="button"
                        className="rounded-full border border-gray-200 px-2.5 py-1 text-[10px] text-gray-600 hover:bg-gray-50"
                      >
                        Edit
                      </button>
                      <button
                        type="button"
                        className="rounded-full p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-400"
                      >
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4">
      {MOCK_QUEUED_CALLS.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <ListOrdered className="mb-3 h-10 w-10 text-gray-200" />
          ...
          {MOCK_QUEUED_CALLS.map((call, idx) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-0.5 text-xs text-gray-500 truncate">{call.objective}</p>
                <span className="mt-1 inline-block rounded-full bg-[#dde9f4] px-2 py-0.5 text-[10px] text-gray-700">
                  {call.agentPersona}
                </span>
              </div>
              ...
              <div className="flex shrink-0 gap-1.5">
                <button
                  type="button"
                  className="rounded-full bg-green-500 px-2.5 py-1 text-[10px] font-medium text-white hover:bg-green-600"
                >
                  Place now
                </button>
                <button
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected ? 'bg-[#dde9f4]' : 'hover:bg-gray-50',
      )}
    >
      {/* Status + direction */}
      ...
      <div className="w-40 shrink-0">
        ...
        {call.counterpartyName && (
          <p className="truncate text-[11px] text-gray-400">{call.counterpartyPhone}</p>
        )}
      </div>
      ...
      {call.scheduledAt ? (
        <div className="flex flex-1 items-center gap-1.5 min-w-0">
          ...
          <span className="truncate text-xs text-gray-500">{call.objective}</span>
        </div>
      ) : (
        <p className="flex-1 min-w-0 truncate text-xs text-gray-500">
      ...
      <div className="w-14 shrink-0 text-right">
        {call.status === 'live' ? (
          ...
          <span className="text-xs tabular-nums text-gray-500">{fmt(call.durationSec)}</span>
        ) : (
    ...
    <div className="flex flex-1 flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-6 py-4">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-light text-gray-900">Calls</h1>
          {liveCount > 0 && (
      ...
      <div className="flex shrink-0 items-center gap-0 border-b border-gray-100 px-6">
        {([
          ...
          <button
            ...
            className={cn(
              ...
              tab === t.id
                ? 'border-[#5aacee] text-gray-900 font-medium'
          ...
          >
            ...
            <span
              className={cn(
                ...
                tab === t.id ? 'bg-[#dde9f4] text-gray-700' : 'bg-gray-100 text-gray-500',
              )}
            >
              {t.count}
            </span>
      ...
      {tab === 'calls' && (
        <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-gray-100 px-6 py-2.5">
          <div className="flex items-center gap-1.5">
            {(['all', 'live', 'pending', 'completed', 'failed'] as const).map((v) => (
              <button
                ...
                className={cn(
                  ...
                  statusFilter === v
                    ? 'border-[#8fc4e4] bg-[#dde9f4] text-gray-800'
            ...
            <div className="flex shrink-0 items-center justify-between border-t border-gray-100 px-5 py-2.5">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="flex h-7 w-7 items-center justify-center rounded-full border border-gray-200 text-gray-500 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-30"
                >
