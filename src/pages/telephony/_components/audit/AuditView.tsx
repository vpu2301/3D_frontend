...
const ACTION_COLOR: Record<string, string> = {
  ...
  'call.completed': 'text-[var(--text-3)] bg-[var(--sand-deep)]',
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-[var(--line-soft)] px-6 pt-5 pb-4">
        <div>
          ...
          <h1 className="mt-1 text-[26px] text-[var(--ink)]">Audit Log</h1>
        ...
        <button
          type="button"
          className="flex items-center gap-1.5 rounded-full bg-[var(--ink)] px-3 py-1.5 text-xs font-medium text-white hover:opacity-85"
        >
      ...
      {/* Compliance banner */}
      <div className="mx-6 mt-4 rounded-[10px] border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-800">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        <div className="divide-y divide-[var(--line-soft)] overflow-hidden rounded-[14px] border border-[var(--line-soft)] bg-white">
          {ENTRIES.map((entry) => {
            const Icon = ACTION_ICON[entry.action] ?? Shield;
            const colorClass = ACTION_COLOR[entry.action] ?? 'text-[var(--text-3)] bg-[var(--sand-deep)]';
            ...
            return (
              <div key={entry.id}>
                <button
                  ...
                  className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[rgba(20,22,26,0.02)]"
                >
                  <div className={cn('shrink-0 rounded-[10px] p-1.5', colorClass)}>
                    <Icon className="h-3.5 w-3.5" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-[var(--ink)]">{entry.action}</span>
                      <span
                        className={cn(
                          'rounded-full px-1.5 py-0.5 text-[10px] font-medium',
                          entry.actorType === 'agent'
                            ? 'bg-[rgba(20,22,26,0.06)] text-[var(--text-2)]'
                            : entry.actorType === 'system'
                            ? 'bg-[var(--sand-deep)] text-[var(--text-4)]'
                  ...
                  <ChevronRight
                    className={cn(
                      'h-4 w-4 shrink-0 text-[var(--text-5)] transition-transform',
                ...
                {isExpanded && (
                  <div className="border-t border-[var(--line-soft)] bg-[var(--sand)] px-4 py-3">
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        <p className="plat-eyebrow">Timestamp</p>
                        <p className="mt-0.5 text-[var(--text-2)]">
                          {new Date(entry.ts).toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })}
                        </p>
                      </div>
                      <div>
                        <p className="plat-eyebrow">Target</p>
                        <p className="mt-0.5 font-mono text-[var(--text-2)]">{entry.target}</p>
                      </div>
                      {entry.detail && (
                        <div className="col-span-2">
                          <p className="plat-eyebrow">Detail</p>
                          <p className="mt-0.5 text-[var(--text-2)]">{entry.detail}</p>
                        </div>
                      )}
                      <div className="col-span-2">
                        <p className="plat-eyebrow">Hash</p>
                        <p className="mt-0.5 font-mono text-[10px] text-[var(--text-5)]">
