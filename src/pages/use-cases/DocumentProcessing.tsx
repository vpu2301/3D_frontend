import { FileText, Download, CheckCircle, ArrowRight } from 'lucide-react';
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

const DocumentProcessing = () => {
  return (
    <div className="min-h-screen dark:bg-[color:var(--bg)]">
      <Header />
      
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-[color:var(--blue-100)] to-[color:var(--blue-100)] pt-20 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="flex justify-center mb-6">
              <div className="bg-[color:var(--blue)] p-4 rounded-[28px]">
                <FileText className="h-12 w-12 text-white" />
              </div>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold text-[color:var(--ink)] mb-6">
              Document Processing
              <span className="text-[color:var(--blue)] block">Automation</span>
            </h1>
            <p className="text-xl text-[color:var(--text-2)] mb-8 max-w-3xl mx-auto">
              Transform your document workflows with AI-powered processing that handles extraction, classification, and validation automatically.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-[color:var(--blue)] hover:bg-[color:var(--blue)]" asChild>
                <Link to="/get-started">Start Processing <ArrowRight className="ml-2 h-5 w-5" /></Link>
              </Button>
              <Button size="lg" variant="outline" asChild>
                <Link to="/watch-demo">Watch Demo</Link>
              </Button>
            </div>
          </div>
        </div>
      </section>

      {/* Features Section */}
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-[color:var(--ink)] mb-4">
              Key Capabilities
            </h2>
            <p className="text-xl text-[color:var(--text-2)]">
              Advanced AI technology for comprehensive document handling
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-[color:var(--paper)] p-8 rounded-[28px] border border-[color:var(--line)] hover:shadow-lg transition-shadow">
              <div className="bg-[color:var(--blue-100)] w-12 h-12 rounded-[16px] flex items-center justify-center mb-6">
                <CheckCircle className="h-6 w-6 text-[color:var(--blue)]" />
              </div>
              <h3 className="text-xl font-semibold mb-4">Data Extraction</h3>
              <p className="text-[color:var(--text-2)]">
                Automatically extract key information from invoices, contracts, forms, and other business documents with 99%+ accuracy.
              </p>
            </div>

            <div className="bg-[color:var(--paper)] p-8 rounded-[28px] border border-[color:var(--line)] hover:shadow-lg transition-shadow">
              <div className="bg-[color:var(--blue-100)] w-12 h-12 rounded-[16px] flex items-center justify-center mb-6">
                <FileText className="h-6 w-6 text-[color:var(--blue)]" />
              </div>
              <h3 className="text-xl font-semibold mb-4">Classification</h3>
              <p className="text-[color:var(--text-2)]">
                Intelligently categorize documents by type, urgency, and routing requirements for streamlined processing.
              </p>
            </div>

            <div className="bg-[color:var(--paper)] p-8 rounded-[28px] border border-[color:var(--line)] hover:shadow-lg transition-shadow">
              <div className="bg-[color:var(--blue-100)] w-12 h-12 rounded-[16px] flex items-center justify-center mb-6">
                <Download className="h-6 w-6 text-[color:var(--blue)]" />
              </div>
              <h3 className="text-xl font-semibold mb-4">Validation</h3>
              <p className="text-[color:var(--text-2)]">
                Cross-reference extracted data against existing systems and flag discrepancies for human review.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="bg-[color:var(--ink)] py-20">
        <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">
            Ready to Automate Your Document Processing?
          </h2>
          <p className="text-xl text-[color:var(--text-4)] mb-8">
            Join thousands of companies saving hours on document workflows every day.
          </p>
          <Button size="lg" className="bg-[color:var(--blue)] hover:bg-[color:var(--blue)]" asChild>
            <Link to="/get-started">Get Started Today</Link>
          </Button>
        </div>
      </section>
      
      <Footer />
    </div>
  );
};

export default DocumentProcessing;
